...
Player.prototype.addPlayerTab = function() {
    ...
    if (this.longestRoad || this.largestArmy) {
        ...
        // Footer: Dynamic Status Banner
    if (this.status) {
        var isAlert = this.status.includes("⚠️");
        var bg = isAlert ? "#ffdddd" : "#e6f7ff";
        var textCol = isAlert ? "#d9534f" : "#005a9e";
        var border = isAlert ? "#d43f3a" : "#91d5ff";
        
        html += "<div class='pc-status-bar' style='background: " + bg + "; color: " + textCol + "; border-top: 1px solid " + border + ";'>";
    ...
    // Add a "Left Game" label over their card
    tab.append("<div style='position:absolute; top:40%; width:100%; text-align:center; font-weight:bold; color:red; transform:rotate(-15deg); font-size:20px; border:2px solid red; background:rgba(255,255,255,0.8);'>LEFT GAME</div>");
}
}

/**
 * Highlights the active player's tab. 
 * Only pulses if it is the local user's turn.
 */
Player.prototype.fillTurnDisplay = function() {
    var tab = $("#p" + this.id + "-tab");
    
    if (currentPlayerTurn === this.id) {
        // Everyone sees the gold border for the active player
        tab.addClass("active-turn-highlight");

        // ONLY this specific player gets the "breathing" pulse effect
        if (this.id === playerId) {
            tab.addClass("active-user-pulse");
        } else {
            tab.removeClass("active-user-pulse");
        }
    } else {
        // Clean up both classes if it's not their turn
...
function parsePlayers(playersData) {
    ...
    for (var i = 0; i < playersData.length; i++) {
        ...
        player.is_active = playerData.is_active;
